...
from tamil_ocr_fork.ocr_tamil_batch.strhub.data.utils import Tokenizer
from tamil_ocr_fork.ocr_tamil_batch.strhub.models.utils import load_from_checkpoint
from tamil_ocr_fork.ocr_tamil_batch.craft_text_detector import (
    load_craftnet_model,
    get_prediction,
...
class OCR_BATCH:
    ...
    def __init__(self,detect=False,
                 ...
                 enable_cuda=True,
                 batch_size=8, # this is the batch size for the parseq model
                 ocr_batch_size=8, # this is the batch size for the whole ocr
                 text_threshold=0.5,
                 link_threshold=0.1,
                 low_text=0.3,
                 details=0,
    ...
    def craft_detect(self,image,**kwargs):
        ...
        prediction_result = get_prediction(
            ...
            poly = False,
            half=self.fp16,
            craft_batch_size=self.ocr_batch_size,
        )
    ...
    def read_image_input(self,image):
        ...
        if type(image) == str:
            ...
            elif len(image.shape) == 4 and image.shape[3] == 3:  # batch rgb
                img = image
            else:
                raise ValueError(f"Invalid image shape: {image.shape}")
        elif isinstance(image, list):
            print(f"List of images of type {type(image[0])} ")
            img = image
        else:
    ...
    def predict(self,image):
        ...
        if isinstance(image,list):
            text_list = []
            if self.detect:
                temp = self.read_image_input(image)
...
if __name__ == "__main__":
    ...
    ocr = OCR_BATCH(detect=True,lang=['english'],text_threshold=0.2,recognize_thres=0.5, low_text=0.2, batch_size=8,ocr_batch_size=8)
    texts = ocr.predict(np.random.randint(0,255,(8, 768, 768,3)))
    print(texts)
